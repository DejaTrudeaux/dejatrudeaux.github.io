...
_.partition = (array, action) => {
    ...
    var myArray = [[], []];
    for (var i = 0; i < array.length; i++){
    //call function (action) for each element in the array passing element, key, array
    ...
    //return an array made of 2 subarrays: an array that contains all values for which something truthy was returned
        }
    //and an array that contains all falsy values when passed through function
    return myArray;
};
...
*/
_.unique = (array) => {
  //return a new array of all the elements from array with duplicates removed
  var myArray = [];
      for (var i = 0; i < array.length; i++){
...
_.every = (collection, action) => {
  ...
  var newArr = _.map(collection, function(element, index, array){
      return (action(element, index, array) === true ? true : false);
    });
   return (_.contains(newArr, false) ? false : true);
};
